...
class COCOEvaluator(DatasetEvaluator):
    ...
    def __init__(self, dataset_name, cfg, distributed, output_dir=None):
        ...
        # Few-shot evaluation properties
    ...
    def _eval_predictions(self, tasks, predictions):
        ...
        if hasattr(self._metadata, "thing_dataset_id_to_contiguous_id"):
            ...
            for result in self._coco_results:
                ...
                assert result["category_id"] in self._metadata.thing_dataset_id_to_contiguous_id, \
                    "{}(type: {}) not in base classes,\n mapping is {} \n base classes are {}".format(
                        result["category_id"], type(result["category_id"]), reverse_id_mapping, self._base_classes)
        ...
        # Few-Shot detection evaluation
            ...
            for split, classes, names in [
                ...
                for task in self._tasks:
                    coco_eval = (
                        _evaluate_predictions_on_coco(
                            self._coco_api, self._coco_results, task, catIds=classes,
                        )
                        if len(self._coco_results) > 0
                        else None  # cocoapi does not handle empty results very well
                    )
                    res_ = self._derive_coco_results(
                        coco_eval, task, class_names=names,
                    )
                    res = {}
    ...
    def _eval_box_proposals(self, predictions, catIds=None):
        """
        ...
        for limit in [100, 1000]:
            for area, suffix in areas.items():
                stats = _evaluate_box_proposals(predictions, self._coco_api, area=area, limit=limit, catIds=catIds)
...
def _evaluate_box_proposals(dataset_predictions, coco_api,
                            thresholds=None, area="all", limit=None, catIds=None):
    """
    ...
    for prediction_dict in dataset_predictions:
        ...
        if catIds:
            gt_classes = torch.as_tensor([obj["category_id"] for obj in anno if obj["iscrowd"] == 0])
            assert gt_classes.size(0) == gt_boxes.tensor.size(0)
            valid_gt_inds = torch.zeros_like(gt_classes, dtype=torch.bool)
            for c in catIds:
                valid_gt_inds = valid_gt_inds | gt_classes == c
        else:
            valid_gt_inds = torch.ones(gt_boxes.size(0), dtype=torch.bool)

        valid_gt_inds = valid_gt_inds & (gt_areas >= area_range[0]) & (gt_areas <= area_range[1])
...
def _evaluate_predictions_on_coco(coco_gt, coco_results, iou_type, kpt_oks_sigmas=None, catIds=None):
    """
